...
public class BatteryDialogActivity extends AppCompatActivity {
    ...
    private void loadChart() {
        ...
        if (diff < (AesPrefs.getIntRes(R.string.DIALOG_CHART_HISTORY_HOURS, Const.DEFAULT_RANGE_IN_HOURS) * 1000 * 60 * 60)) {
            ...
            hours = AesPrefs.getIntRes(R.string.DIALOG_CHART_HISTORY_HOURS, Const.DEFAULT_RANGE_IN_HOURS);
        }
        if (hours == 0 || hours == 1) {
            tvHeader.setText(getString(R.string.last_hour));
        } else {
            tvHeader.setText(MessageFormat.format("{0} {1} {2}", getString(R.string.dialog_chart_header), hours, getString(R.string.hours)));
        }
